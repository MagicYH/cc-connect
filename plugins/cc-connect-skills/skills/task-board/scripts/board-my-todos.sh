...
# 角色匹配兼容纯角色名与"角色(Bot名)"标签；群chatID 兼容 Group 字段(取 .id)与文本
set -euo pipefail
...
list_rows | jq -r --arg role "$ROLE" --arg cutoff "$CUTOFF" '
  ...
  def fchat: ($f | index("群chatID")) as $i | $r[$i] | if type=="array" then (.[0]//"" | if type=="object" then (.id // "") else . end) elif .==null then "" else . end;
