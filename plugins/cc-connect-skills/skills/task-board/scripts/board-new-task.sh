#!/usr/bin/env bash
# board-new-task.sh <主任务> <工作群> <角色> <子任务> [来源record_id] —— 建待办行（创建人=自己）
# 角色自动映射为"角色(Bot名)"标签（board.env 的 BOT_LABEL_*）；工作群 写入 Group 字段格式。
